-- Forward-only Club Player of the Month history, requested 2026-08-14 —
-- same lazy, cron-free snapshotting pattern as monthly_leaderboard_snapshots.
-- Written by the same snapshot_month_end_leaderboard() call from
-- Leaderboard.tsx, so no new client trigger is needed.

create table public.monthly_club_player_awards (
  year_month text primary key,
  player_id uuid not null references public.players(id) on delete cascade,
  games int not null,
  wins int not null,
  win_pct numeric not null,
  rating_gain numeric not null,
  created_at timestamptz not null default now()
);

alter table public.monthly_club_player_awards enable row level security;

create policy "club player awards readable by any logged-in member"
  on public.monthly_club_player_awards for select
  using (auth.role() = 'authenticated');

-- No direct client insert/update policies — written only by the security
...
begin
  ...
  -- reliability (win %, 3+ games only) and improvement (rating gained)
  -- as the live version on the Leaderboard page, just computed once for
  -- the month that just finished instead of live every page load.
